...
struct FluidBlocks
{
    ...
    std::unique_ptr<std::mutex> m_frontbuffer_mutex;
    std::unique_ptr<std::mutex> m_backbuffer_mutex;
...
public:
    ...
    inline void swap_buffers()
    {
        ...
        // the backbuffer
        std::unique_lock<std::mutex> lock(*m_frontbuffer_mutex);
        m_back_cells.swap(m_front_cells);
        m_backbuffer_mutex.swap(m_frontbuffer_mutex);
    }
};
