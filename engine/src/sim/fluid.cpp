...
FluidBlocks::FluidBlocks(const unsigned int block_count_per_axis,
    ...
    m_frontbuffer_mutex(new std::mutex),
    m_backbuffer_mutex(new std::mutex)
{
...
void Fluid::coordinator_impl()
{
    ...
    while (!m_terminated) {
        ...
        std::unique_lock<std::mutex> buffer_lock(*m_blocks.m_backbuffer_mutex);
...
void Fluid::to_gl_texture() const
{
    ...
    std::unique_lock<std::mutex> lock(*m_blocks.m_frontbuffer_mutex);
